...
async function main() {
  ...
  for (const row of baserowRows) {
    ...
    // Найти соответствующую запись в Payload по slug
    const existingDoc = result.docs.find(doc => doc.slug === slugFromRow);

    if (!existingDoc) {
      console.log(`No matching record found for slug: ${slugFromRow}`);
      continue;
    }
    ...
    console.log(`✅ Updated: ${slugFromRow}`);
  }
