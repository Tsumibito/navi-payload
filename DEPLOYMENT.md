...
> Канонический контракт инфраструктуры и миграций: `INFRASTRUCTURE.md`.
...
`push: false` запрещает неявное изменение схемы. Автоматический запуск обеспечивает отдельная настройка `prodMigrations: migrations`: при старте контейнера Payload применяет зарегистрированные миграции по порядку и останавливает запуск при ошибке. Реестр проверяется командой `npm test`.
